...
module.exports =
  selector: atom.config.get('autocomplete-playcanvas.FileType')
  inclusionPriority: 0
  ...
  loadPlaycanvas: ->
      fs.readFile path.resolve(__dirname, '..', 'playcanvas.json'), (error, content) =>
          return if error
          filetype = atom.config.get('autocomplete-playcanvas.FileType')
